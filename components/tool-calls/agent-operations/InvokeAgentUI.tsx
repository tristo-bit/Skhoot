/**
 * Invoke Agent UI Plugin
 * 
 * Displays agent invocation results.
 */

import React, { memo, useState, useMemo } from 'react';
import { Copy, Check, Bot, CheckCircle, XCircle } from 'lucide-react';
...
export const InvokeAgentUI = memo<ToolCallUIProps>(({ 
  toolCall,
  result,
}) => {
  ...
  const agentId = toolCall.arguments.agent_id || 'unknown';
        ...
        <div className="p-3 rounded-xl border border-glass-border glass-subtle">
          <div className="flex items-center gap-2 mb-2">
            ...
            {agentResult.success ? (
              ...
              <XCircle size={12} className="text-red-600 ml-auto" />
            )}
          </div>
          <div className="text-[11px] text-text-primary bg-black/10 dark:bg-white/5 p-2 rounded-lg">
            {agentResult.response || agentResult.message || 'No response'}
          </div>
        </div>
      ) : (
...
InvokeAgentUI.displayName = 'InvokeAgentUI';
